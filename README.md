...
Failed or weak checks include **How to resolve** guidance, step-by-step remediation, safe DNS templates, copy buttons, and warnings where provider-specific values are required. SPF and DKIM templates intentionally use placeholders when the app cannot safely infer the outbound sender or signing key. DMARC guidance follows a phased monitoring → quarantine → reject rollout.
